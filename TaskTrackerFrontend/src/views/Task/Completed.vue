...
const getCompletedTasks = async (): Promise<void> => {
  try {
    const response = await axios.get('http://127.0.0.1:8000/api/completed', {
      headers: {
        Authorization: `Bearer ${authStore.token}`,
      },
    });
    console.log('Completed tasks retrieved');
    ...
    console.error(
      `[FETCH-COMPLETED-ERROR] An error occurred while fetching completed tasks: ${error}`,
    );
  }
};
</script>
...
<template>
  <SidebarProvider>
    ...
    <main class="w-full">
      ...
      <h1 class="text-2xl text-center">Completed Tasks</h1>
